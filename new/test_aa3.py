...
N = 30000
for MAXITER in [0,1,2,5,10,20]:
    dr_y = np.zeros(2 * self.m)

    losses = []
    # xs = []
    # steps = []
    for i in range(N//( 1 + 2 * MAXITER)):
        # xs.append(np.copy(dr_y))
        base_step = self.douglas_rachford_step(dr_y)

        losses.append(np.linalg.norm(base_step))
        print(losses[-1])
        if losses[-1] < 1e-10:
            print(f'converged in {i} iterations')
            break

        J = self.douglas_rachford_step_derivative(dr_y)
        zio = sp.sparse.linalg.lsmr(J, base_step, x0=-base_step, maxiter=MAXITER)[0]
        # zio = sp.sparse.linalg.lsqr(J, base_step, x0=-base_step, iter_lim=MAXITER)[0]
        dr_y -= zio
    else:
    ...
    plt.semilogy(np.arange(N // (1 + MAXITER * 2)) *(1 + MAXITER * 2), losses, label=MAXITER)
